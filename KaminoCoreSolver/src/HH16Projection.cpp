	...
	*/
	...
	for (it = 1; it < nt; it++)
	{
		for (ip = 1; ip < np - 1; ip++)
		{
			...
			double factor = invRad * 0.5 * invGridLen * invSin * rho;
			fReal u_left = uPhi->getValueAt(ip - 1, it);
			fReal u_right = uPhi->getValueAt(ip + 1, it);
			fReal u_up = vTheta->getValueAt(ip, it + 1);
			fReal u_down = vTheta->getValueAt(ip, it - 1);
	...
	for (it = 1; it < nt; it++)
	{
		...
		fReal u_right = uPhi->getValueAt(0, it);
		fReal u_up = vTheta->getValueAt(np - 1, it + 1);
		fReal u_down = vTheta->getValueAt(np - 1, it - 1);
		f[ip + it*(np + 1)] = factor * (u_right - u_left + u_up * sin_up - u_down * sin_down);
	}
	...
	for (it = 1; it < nt; it++) {
		...
		fReal u_left = uPhi->getValueAt(np - 1, it);
		fReal u_right = uPhi->getValueAt(1, it);
		fReal u_up = vTheta->getValueAt(0, it + 1);
		fReal u_down = vTheta->getValueAt(0, it - 1);
