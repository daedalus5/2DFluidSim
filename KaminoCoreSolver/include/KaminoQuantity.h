...
public:
	...
	fReal& accessValueNextAt(size_t x, size_t y);
...
private:
	...
	void mapVToSphere(Eigen::Matrix<float, 3, 1>& pos, Eigen::Matrix<float, 3, 1>& vel) const;
public:
	KaminoParticles(fReal particleDensity, fReal radius, fReal h, KaminoSolver* parentSolver, size_t nPhi, size_t nTheta, std::string densityImage, Eigen::Matrix<fReal, 3, 1>* colorMap);
	~KaminoParticles();
